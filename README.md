...
✅ **实时 Hook 通知** — Claude 需要审批时桌面弹窗提醒，点击直接跳转  
✅ **会话实时统计** — 每个终端显示当前会话 Token 用量、费用、工具调用  
✅ **历史 Diff 回看** — 统一查看所有历史会话的代码变更，支持跳回触发消息  
✅ **用量分析看板** — 多维度统计（热力图、趋势图、效率散点）  
✅ **项目级供应商切换** — 一键切换 Claude 后端（官方 / 中转 / 自建），无需手动改配置  
✅ **灵活分屏布局** — 自由的终端分屏 + Tab 跨 pane 拖拽  
