...
* [x] 打开多个终端直到 Tab 溢出时，Tab 栏仍保持单行，不换行、不挤压主内容区。
* [x] 溢出时显示左右导航按钮，点击后 Tab 列表横向滚动。
* [x] 通过点击、下拉选择、快捷键切换 Tab 时，当前 Tab 自动滚动到可见区域。
* [x] 溢出下拉可以看到全部终端 Tab，并能切换到指定 Tab。
* [x] 当前 Tab 的关闭按钮可见；非当前 Tab 不常驻打扰，但 hover/focus 时可关闭。
* [x] 拖拽排序、右键菜单、关闭其它终端、新建终端、分屏入口不回退。
* [x] `npx tsc --noEmit` 通过。
