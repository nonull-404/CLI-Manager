...
### 历史与统计
- **ccusage 用量分析布局优化**：宽屏下将峰值日摘要与 Token 构成并排、按天 Token 热点图置于其下方，模型用量排行独立放在右侧，避免短内容卡片被排行图表等高拉伸产生大面积留白。
- **历史用量分析卡片宽度调整**：宽屏下加宽活跃时段分布柱状图并缩短 Token 构成卡片，使时段数据更易查看；中小屏布局保持不变。
- **来源对比图形调整**：来源对比改为 Claude/Codex 双环形图，为每个来源补充 Token 类型图例、数量与占比明细，并在来源标题前显示品牌彩色图标。
- **请求日志界面优化**：筛选条件、快捷日期、高级筛选和查询操作收拢为紧凑工具栏，四项汇总指标使用带间距的独立卡片；表格统一居中并补充来源品牌图标、来源/项目/模型快捷筛选、异常用量提示，将会话 ID 复制与查看会话合并到固定操作列。

### 设置
- **Hook 桥接开关布局调整**：Claude Code 与 Codex CLI 的桥接开关移动到对应桥接标题行，并保持开关操作与区域折叠互不干扰；桥接关闭时隐藏安装状态、折叠图标和桥接详情。

### 修复
- **历史用量分析图表焦点框修复**：移除鼠标点击 Recharts 图表后出现的浏览器默认黑色边框，并为键盘导航保留主题色焦点提示。
- **历史会话热力图提示重复修复**：24 小时会话热力图悬浮提示改为每个时段仅显示一次会话数与消息数，不再因散点图横纵坐标生成重复内容。
- **Claude 状态栏 Powerline 符号修复**：WebView 直接加载应用内置符号字体，不再依赖 Windows 用户字体缓存，修复实时预览、Powerline 选项和应用内终端中的分隔符与端帽显示为方框的问题。
- **历史会话文件定位方式调整**：会话详情中的“复制定位”改为“打开文件”，点击后使用系统文件管理器打开所在文件夹并选中对应的历史会话文件。
